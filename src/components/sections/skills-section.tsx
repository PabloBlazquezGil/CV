    ...
    <section id="skills" className="container py-10 md:py-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
        <Card className="shadow-lg bg-card/50 group hover:bg-primary transition-all duration-300 cursor-pointer">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-xl font-headline group-hover:text-primary-foreground">
          ...
          <CardContent className="flex flex-wrap gap-2">
            {skills.technical.map((skill, index) => (
              <Badge key={index} variant="secondary" className="text-sm px-3 py-1 group-hover:bg-primary-foreground group-hover:text-primary group-hover:hover:bg-primary-foreground/90">
                {skill}
              </Badge>
            ))}
          </CardContent>
          ...
          <CardContent className="space-y-4">
            {skills.languages.map((lang, index) => (
              <div key={index} className="flex justify-between items-baseline gap-4 group-hover:text-primary-foreground">
                ...
                <span className="text-muted-foreground text-right group-hover:text-primary-foreground/70">{lang.level}</span>
              </div>
            ))}
          </CardContent>
        </Card>
